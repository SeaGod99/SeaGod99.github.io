...
| 🎣 釣魚紀錄追蹤 | `/tools/fishing/` | 1414 種台服已開放的魚查詢（釣場/餌/時間/天氣），追蹤已釣進度 |
| ⛏️ 採集紀錄追蹤 | `/tools/gathering-log/` | 733 個採集節點，追蹤已採物品進度 |
| 🏝️ 無人島開拓查詢 | `/tools/island/` | 工坊生產品（時數／價值／製作類型／素材，類型可點篩選）、採集地圖＋素材反查合併頁（點素材標出採集區域・所需工具・重疊區）、動物時鐘（出現時段／天氣即時倒數・體型與捕捉道具）、開拓等級表 |
| 📊 市場查價 + 比價 | `/tools/market/` | 陸行鳥 DC 即時價格、跨服比價、製作清單原料樹狀圖與買／做成本試算 |
| 🔨 製作模擬器 | `/tools/crafting-sim/` | 排一串製作技能，逐步算出作業、品質、耐久、CP 與 HQ 機率，並產生台服用字的遊戲內巨集 |
...
### 🏝️ 無人島開拓（三期皆已上線）
- 🐑 動物時鐘 43 種全數上線；id 30–43 名稱為簡中譯名轉繁（非台服官方，遊戲內用字可能不同）
- 🏷️ 建築／製作類型／地區／分類名目前是簡中轉繁，日後可用台服遊戲內用字取代
