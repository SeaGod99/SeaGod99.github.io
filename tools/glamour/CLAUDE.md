...
**繁中版目前尚未開放跨職業幻化（Glamour Plate 跨職業功能）**（2026-08-11 台服升上 7.21 後**仍然沒有**）。因此：
- 某套裝若含有特定職業限制的裝備（如盾衛職業的靴子用在治療職業套裝），在繁中版無法實際幻化
- 裝備限制欄位仍顯示 DB 的正確資料，供參考
- ⚠ **`index.html` 的 `jobCompatOK()`（「整套需有單一職業能同時穿上所有件」）不要拿掉**——
  它就是這條遊戲限制的實作。拿掉會讓「🇹🇼 繁中版可幻化」多放行 839 套玩家實際做不出來的配裝
  （社群 815、精選 24）。台服哪天真的開放跨職業混搭再改，並同步更新本節與按鈕 tooltip。
...
> ⚠ **不要拿 Ollama／Haiku 的輸出直接注入 `ocr_cache.json`。** 同一批圖對日文道具庫實測：
> ollama `qwen2.5vl:7b` 58%、Claude Haiku 39%、Claude 主模型親讀 99%
> （細節與搶救法的失敗數據見 [知識庫 §4.20](../../docs/專案慣例與記憶.md)）。
> 錯名進了快取不會報錯，只會讓 `mirapri_visible` 把玩家真的穿著的裝備靜靜隱藏。
> **這支腳本的定位是「先掃一遍找出可疑的套」，逐字轉錄要由 Claude 親讀並回查道具庫。**
> 低階模型倒是很適合做二元分類（「這張圖有沒有裝備清單面板」實測 11/11 正確）。
