...
>
> **week 441「知性蠻族工匠（Mindful Master）」** 已於 2026-07-15 依 SOP 更新（verified 版）。該次新學到：①`data/items.json` 的 marketable 在 `.data` 陣列非頂層；②verified easy80 可少到 1 件（頭部＋染色）；③前端時鐘會領先 API 一個空窗週（收榜後到下週五開放前），此時只能更新到上一週 verified、頁面顯示為存檔。
