...
修法寫在 [`scripts/lib/game-sources.mjs`](../scripts/lib/game-sources.mjs) 的 `currencyGate()`：**只要兌換貨幣不是金幣（id 1），就一定掛門檻**，沒有白名單。認得出種類的給準確標籤與權重（零式戰利品 raid=5、神典石 tome=2、軍票／金碟 grind=2、戰績 pvp=3、限時活動 event=4…），認不出來的給通用門檻 currency=3。寧可高估代價，也不要再把高難度內容講成免費。
