...
**最後更新**：2026-08-26（**製作模擬器大改**：新增 ⚡ 自動求解（兩階段 beam search，只用必定成功／不吃球色的技能，四套範本情境全部贏過手解值）、料理／藥品加成（`craft-consumables.json`，料理 49／藥品 12）、我的循環（具名儲存／載入／刪除可復原）；並依回饋做了巨集不加雙引號、作業與品質畫面封頂、製作結束後技能停用、開場技能獨立一組、移除恆為「一般」的作業狀態欄、寬螢幕兩欄各自捲動＋結果條收進卡片、說明文字精簡。量到並修掉標題列整排錯位（改用 subgrid）與「分寬度覆寫被後面的規則蓋掉」。`validate-craft-sim` 76 → **104 項全過**。08-25（幻化配裝圖鑑資料更新：Mirapri 6,918→6,933、社群顯示 **6,887 套**，空殼重建 2,277 套、逐件染色 6,145 套；16 張新圖由 Claude 逐張親讀，**名稱 84/84、染色 77/77 全中**，對 API 清單 79/80——唯一的差是 §4.21 汙染件（圖上沒有）。三件糊掉／被切掉的標籤都回查道具庫定案（紫水小袴【間士】／アスフォデロース・ディフェンダースカート／大法師手紋）；**新增製作模擬器** `/tools/crafting-sim/`：選配方＋填工匠數值→排製作技能循環→逐步算出作業／品質／耐久／CP 與 HQ 機率，並產生台服官方用字的遊戲內巨集。36 個技能名全部取自台服語系檔（**不簡轉繁**），CP 與解鎖等級對 XIVAPI **36/36 相符**；公式移植自 Teamcraft 模擬器（MIT）並以**其官方測試案例**回歸——首跑 67/68 抓到唯一漏掉的規則（專心致志在非高品質下被集中加工／集中製作／秘訣用掉要消失），補上後連同四套內建範本 **76 項全過**。作業狀態的官方用字是從技能與道具說明裡挖出來的（高品質／最高品質／結實／安定／高效／長持續／大進展），**查不到台服字串的四種不硬翻**、改用效果描述。內建範本憑印象排的四套實測全做不完，改用離線兩階段 beam search 解出來再寫死並納入回歸。另量到並修掉一個看不出來的顯示 bug：四條進度條的填色寬度是 0（`<span>` 是 inline，不吃 width）。三種寬度水平溢出 0、亮暗兩主題對比未達 AA 者 0。08-23 幻化配裝圖鑑**修好逐件染色的第二槽**：可染裝備有兩個染色槽，兩槽同色時整條被去重吃掉、第二槽顯示成「—」，照著配的人不會知道第二槽也要染——全庫補回 **250 件**，改前改後逐件 diff 的四類非預期變化全部 0。08-22 幻化配裝圖鑑資料更新：Mirapri 6,912→6,918、社群顯示 **6,871 套**，空殼重建 2,261 套、逐件染色 6,132 套；6 張新圖由 Claude 逐張親讀，**名稱 30/30、染色 50/50 全中**，兩件糊掉的標籤都靠回查道具庫定案（辨天袴下／ユウェヤーワータ・スカウトグローブ）。另量到一個既有行為未改：同一件兩槽同色時第二槽會被去重成「—」，全庫 255 件。08-20 幻化配裝圖鑑兩輪更新：Mirapri 6,839→6,912、社群顯示 **6,865 套**，空殼重建 2,255 套、逐件染色 6,126 套；73 張新圖全部由 Claude 逐張親讀，名稱與染色**兩輪都是 100% 命中**。第二輪看到 §4.21「API 清單把相鄰投稿黏在一起」的完整累加序列（同一位投稿者連四筆 5→10→12→空殼），清單完全不能當真值。前一輪細節：Mirapri 6,839→6,900、社群 6,853 套，空殼重建 2,243 套、逐件染色 6,115 套、item_sources 1,001 種/11,892 件；61 張新圖由 Claude 逐張親讀，**名稱 326/326、染色 340/340 全中**——這是 08-12 修好染色白名單後的第一次全新判讀，7.21 新色全部正確落地。health_check 全過、開頁零錯誤、無解清單零變化；08-12 **染色對照從手工凍結改成主庫產生**：白名單凍在 146 色，害 7.21 的 11 個新色不是留白而是被模糊比對指派成最像的舊色（メタリックルビーレッド 顯示成寶石紅 58 件）；新增 build_dye_names.py 並給 snap_dye 補兩道防線後，前端補上空白染色 587 件、修正錯色 544 件、移除不可信殘字色 165 件。同日幻化配裝圖鑑資料更新：Mirapri 6,828→6,840、社群顯示 6,793 套，空殼重建 2,182 套、逐件染色 6,005 套、item_sources 999 種/11,878 件；12 張新圖由 Claude 逐張親讀，名稱**對日文道具庫 63/63 全中**，health_check 全過、開頁零錯誤。這輪 enrich 是用 7.21 主庫重跑的，社群套的繁中名同時補上一批新開放條目；08-12 另有園藝配種頁兩輪（改成解最短路徑＋補用途與照料時程），詳見更新紀錄；08-11 **台服版本閘門 7.15 → 7.21**：依 Teamcraft 台服語系檔實測，台服這次把國際服 7.2＋7.21 併成一次更新，門檻設 7.21 才不會把 526 筆內容繼續關在門外。重點不是版本號而是繁中名——`tw-items.msgpack` 原本是沒有腳本產它的一次性快照、卡在 7.15，新增 `build-tw-items-msgpack.mjs` 重建後 items.json 43,748 → **45,548**，順帶修掉 80 個混進來的簡中用語；再以新增的 `patch-tw-names.mjs` 補齊魚 8／園藝 2／鳥鞍 1／隨從 5 筆缺名，`validate-data` 0 error。幻化配裝圖鑑同步重建跟上，缺繁中名的裝備件少 1,022 件、「繁中版可幻化」套數 +347，見更新紀錄⑤；08-09 幻化配裝圖鑑再更新一輪：Mirapri 社群投稿 6,774→6,828、社群顯示 6,781 套，空殼重建 2,170 套、逐件染色 5,996 套、item_sources 985 種/11,876 件，健檢全過、開頁零錯誤。本輪**實測 Haiku 判讀**：名稱只有 39% 對得上道具庫（比 ollama 的 58% 還差，Claude 親讀 99%），改用 API 清單吸附搶救後覆蓋率仍只有 57%，整批捨棄；但 Haiku **在「有沒有裝備清單面板」的二元判斷上 4/4 正確**，該 11 張予以採信——分工結論是低階模型可做圖片分類、不可做逐字轉錄。另發現 **Mirapri API 會把相鄰投稿的清單黏在一起**（#048 的清單與圖完全不相干）與 **7.x 臉部飾品整類在道具庫缺本體**（47 筆解鎖書對應 27 筆查無），見更新紀錄；08-08 幻化配裝圖鑑資料更新完成：Mirapri 社群投稿 6,628→6,774、官方套裝 1,971→1,975 套、多語裝備庫重抓 28,963 件；**249 張新圖判讀全數完成**（206 張 Claude 視覺親讀＋43 張保留 ollama），並**量化了 ollama OCR 的兩個系統性失效**（無面板圖會捏造、空殼套錯了整套都錯）、修掉注入腳本「只跳過不刪除」的漏洞；08-03 全站 UI/UX 一致性體檢：亮/暗兩主題 26 頁的文字對比 168 類未達 AA **全數修完歸零**、觸控目標加 44px 下限、內容欄寬 9 種收斂成 3 階 token、頁尾 19/26 → 26/26 並加 skip link、手機水平溢出歸零；08-01 全站設計 token 收斂到 `assets/css/tokens.css`、修掉亮色 `--accent` 全站洩漏、幻化配裝圖鑑併入主題／頂列系統、加上全站焦點環與 `prefers-reduced-motion`；07-29 冒險者小隊計算機 9 個算錯／顯示錯的 bug 全修＋任務列表改表格、SW `CACHE_VERSION` 跨機無限漂移的根因（autocrlf）修掉、職業名三份表統一、精選取得方式補 224 處、主庫 equip 缺口修補；07-28 幻化配裝圖鑑資料併回主庫：刪 105MB 重複、修 570 筆錯名、加 .gitattributes 修 msgpack 損壞；另有資料缺口／UX 一致性／效能 三章＋副本庫 386→520 座等，見更新紀錄）
