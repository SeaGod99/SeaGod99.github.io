...
| 4.10 | 無人島開拓查詢 | `/tools/island/` | 完成（07-23 新增第一期，07-24 加入第二期）：四分頁——**工坊生產** 81 筆（時數／價值／每小時／主題／工房等級，可依時數與**製作類型（主題）**篩選、四種排序）／**素材反查** 109 素材依取得方式分組，顯示採集**區域**（中心＋半徑）、農場種植、牧場產出（列出具體動物並可跳轉）、製作，並反查用途／**動物時鐘** 29 種動物的出現 ET 時段與天氣，即時倒數「現在可捕獲／下次出現」，天氣為純計算不連 API／**採集地圖** 島嶼底圖＋**26 個採集區域**圓圈（48 種素材共用，一區可採多種）、地上洞窟分層。分頁與篩選同步網址可分享 |
...
4. 無人島攻略工具：**第一期（素材／工坊查詢）＋第二期（動物時鐘）已上線**，見 `docs/無人島攻略工具規劃.md`。11 個 `data/island-*.json` 已產出、validate 全過；素材/工坊生產/建築素材/收購/等級**台服繁中名 100% 覆蓋**。動物 29/43 已具名並帶出現條件（來源見 `island-names-tw.json` 的 `_animalSource`），**剩 14 筆（後續版本追加）與建築名 25／主題名 16／分類名 10／地區名 6 仍待人工從台服抄錄**（填 `data/island-names-tw.json`）。第三期＝開拓進度／解鎖表，資料已備、只差建築名
5. 其餘收藏追蹤頁（髮型）
...
- **2026-07-24（無人島第二期上線：動物時鐘）**：使用者提供 [steamxo 的動物一覽](https://www.steamxo.com/2023/01/05/2700775)（「雖然是無官方確認，但資料應該是正確的」），卡了三天的動物名與出現條件解鎖。
  ...
  - **修正了原文三處錯誤**（全記在 `_animalSource.knownIssues`）：① 星點栗鼠「9am-12am」應為 9am-12pm（全表時窗皆 3 小時）；② 猴面雀那列時間與天氣兩欄對調；③ 雄羚羊／公洞山羊的第二項報酬與 datamine 不符——**報酬一律以 `MJIAnimals` 為準**，原文報酬只用於對位。
