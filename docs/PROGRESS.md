...
**最後更新**：2026-08-27（幻化配裝圖鑑**清掉 47 套重複投稿**：新增 `check_duplicates.py`，用裝備簽章 × md5/dHash 交叉找重複，只處理「裝備一樣＋圖也一樣」那類；保留規則是**染色資訊最完整者優先**、平手才取較新（照時間取會丟掉 6 組較完整的染色）。社群 6,907→**6,860 套**，`item_sources` 不變＝沒少掉任何裝備。同日稍早：Mirapri +20，20 張新圖判讀**名稱 120/120、染色 120/120 全中**，並學到「標籤糊掉時先找同投稿者的英文版截圖」（知識庫 §4.34））
