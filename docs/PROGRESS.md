...
| # | 工具 | 路徑 | 狀態 |
|---|------|------|------|
| 4.1 | 市場查價 + 比價 | `/tools/market/` | 完成（Universalis 即時價、跨服比價、製作原料樹、URL 深連結 `#item=`；收藏頁「💰 市場行情」連此。**08-09 總採買清單改為可折疊的縮排「製作計畫」**：同層相同物品合併、任一列可勾「我已經有了」並自動剪掉底下整段。**08-10 大改**：成本引擎改為逐筆吃掉掛單、新增「💰 賺錢排行」分頁、價格走勢與百分位、製作利潤試算、8 服一站購足比較、不可交易素材接站內取得管道、自訂實際買價、localStorage key 遷入 `ffxiv_` 前綴。**08-11**：頁面邏輯抽成 `tools/market/market.js`（index.html 3,636 → 776 行）、材料排序改為固定依物品 ID、「我的清單」改成可展開品項的折疊卡） |
