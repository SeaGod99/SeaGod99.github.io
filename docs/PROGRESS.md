...
| # | 工具 | 路徑 | 狀態 |
|---|------|------|------|
| 4.1 | 市場查價 + 比價 | `/tools/market/` | 完成（Universalis 即時價、跨服比價、製作原料樹、URL 深連結 `#item=`；收藏頁「💰 市場行情」連此。**08-09 總採買清單改為可折疊的縮排「製作計畫」**：同層相同物品合併、任一列可勾「我已經有了」並自動剪掉底下整段，並修掉「子樹含買不到的材料就整批不列」的 bug） |
...
- **2026-08-09（市場查價：「總採買清單」改成可折疊的縮排「製作計畫」，並讓「已有」真的參與計算）**：只動 `tools/market/index.html`，未動 `data/`／`assets/`（HTML 走 SW 的 network-first，不需 bump 版本）。
  ...
  - **已知未處理（刻意不擴大範圍）**：「簡易清單」的 `.sr-total` 與 320px 下的分頁鈕仍會造成頁面水平溢出，**改動前就有**，與製作計畫無關。
