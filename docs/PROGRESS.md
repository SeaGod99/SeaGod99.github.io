...
- **2026-09-07（幻化配裝圖鑑：官方套裝更新，示意照首次 100% 覆蓋）**：只動 `tools/glamour/`，主庫未動、不需 bump SW。跑的是官方套裝那條線（`build_item_fallback` → `build_sets --fetch` → `fetch_icons` → `fetch_set_photos --retry-miss` → 重建 → 健檢）。
  - **套數沒變、但名稱大幅補齊**：上游 `MirageStoreSetItem` 仍是 1,170 列、官方套裝仍 **1,975 套**（沒有新套裝箱）。真正的收穫在 `build_item_fallback.py` 重掃 XIVAPI：件數同樣 28,963，但**有繁中名的從 26,264 → 26,949（+685）**、無繁中 2,699 → **2,014**。台服補了一批繁中名，套裝與裝備清單同步跟上。
  - **官方示意照補到 1,974 → 1,975/1,975，0 套對不到**（首次 100%）。缺的那套是 `mirage:52660`＝**Zero's Luminary Attire**（7.5 商城套），先前兩個 wiki 都還沒圖，這次 consolegameswiki 上了 `Zero's Luminary Attire Female.jpeg`；順帶抓到 `source-type: Premium` ＋ `obtain-by: Online Store`，取代原本籠統的「🎁幻化套裝箱」。icon 維持 8,780/8,780 全覆蓋、本次下載 0 張。
  - **順手清掉一個會一直復發的提示**：`build_site.py` 報「以 DB 校正欄位（精選）：7 件（來源檔已過時）」——那 7 件正是這批新繁中名的受益者（`ベルトレザー・ベレー`→皮帶貝雷帽、`アーゼマ・タンガ`→阿澤瑪夏褲…）。跑 `normalize_curated_from_db.py --apply` 寫回 `data/curated_outfits.json`，**7 處全是「原本空白 → 補上官方繁中名」，沒有覆寫任何既有值**。不寫回的話每次建置都會再校正一次、來源檔永遠落後。精選缺繁中 106 → **99 件**。
  - 前端：`official_sets.js` 1,975 套、`item_db.js` 6,385 件、`item_sources.js` 1,003 種/11,911 件（社群資料本輪未動，`mirapri_outfits.js` 重建後位元組不變）。`health_check` **全部通過**。
  - **驗收**：headless Edge ＋ CDP 量 1440／375，**水平溢出 0、console 零錯誤**；另切到**官方套裝檢視**量了一次——60 張卡片、60 張圖、**載入失敗 0**。四個本輪新補的繁中名（皮帶貝雷帽／阿澤瑪夏褲／革新者大地工作坎肩／黯雲詠咒長角）在套裝檢視都搜得到。
- **2026-09-05（幻化配裝圖鑑資料更新：Mirapri +58）**：只動 `tools/glamour/`，主庫未動、不需 bump SW。
  - `pipeline.py all` 查到 **6,999 → 7,057 筆**（+58），接 `enrich`。壓縮 58 張、縮圖 58 張、兩層化 58 張（縮圖／卡片層覆蓋各 **7,164/7,164**）。主庫仍是 7.21、未改版。
  - 待判讀 68 張 ＝ 58 張新圖 ＋ 10 張確認過沒有裝備清單面板的舊圖（那 10 張刻意不寫快取，每輪都會再被列出來，屬正常；本輪複驗仍全數無面板）。58 張新圖全部由 Claude 逐張親讀、每件回查道具庫：**名稱 313/313（100%）、染色 335/335（100%）**，`OCR無解清單.md` **零變化**。
  - **唯一一件回查才定案的**：#056 的武器讀作 `不老鋼太刀`，道具庫查無；該投稿是日英雙語標籤，英文 `High Steel Tachi` **唯一命中 `不変鋼太刀`**（id 18046）——「変」被我誤讀成「老」。這正是 [§4.34](專案慣例與記憶.md) 的用法：**英文名回查是精確查表，比字形猜測可靠**。
  - **§4.21「API 清單累加汙染」這輪出現 3 次**（#016／#018／#034）：件數 10/10/11，圖上實際只有 5/5/5 件，前半段都是前一筆投稿的清單。因 `src=="claude"` 一律 force_recon，判讀直接把汙染清單修剪掉。
  - **§4.34 的日英對照這輪有一組完整實例**：#054（日文版）與 #055（英文版）是同一位投稿者的同一套，兩張各自獨立判讀後**逐件 5/5 完全吻合**（含每一格染色），等於互相驗證。兩筆都是獨立投稿，各自寫進快取。
  - **兩套的 API 清單有圖上沒標的件**（#050 的 `紅蓮祭小袴`、#060 的 `大龍脈耳飾【武士】`）。#050 的腿部標籤位置只有 FFXIV 浮水印，已放大＋自動對比確認**沒有藏標籤**；依既有慣例「判讀為權威、圖上沒畫的不收」處理，這是「直接隱藏」策略的已知取捨。
  - 快取 7,089 → **7,147 筆**；空殼重建 2,343 → **2,401 套**（58 套全部依圖重建）、逐件染色 6,159 → **6,213 套**；社群 **6,965** 套（原始 7,057 扣掉人工決定移除的 92 套）／官方 1,975 套／精選 95 套；`item_db.js` 6,364 → **6,385** 件裝備；`item_sources.js` 11,906 → **11,911 件**（來源種類維持 1,003）。`build_site.py` 的**緊湊編碼無損還原驗證通過**、`health_check` **全部通過**。
  - **驗收**：headless Edge ＋ CDP 量 1440／375 兩種寬度，**水平溢出 0、console 零錯誤**；前端搜「硬鋼太刀」→#056、「闊腿褲」→#066、「昕金工作衫」→#049 都撈得到。`夜桜脚衣`（7.5 新裝）已進 `item_db.js`，**繁中名留空是正確的**——台服未實裝，依 [§4.5](專案慣例與記憶.md) 不用日文或英文補。
...
- **2026-09-05（時尚品鑑 week 449 更新，並補上成本模型最後一個「免費」破口）**：更新到 **week 449「遠東獵人」（Hunter from the Far East）verified**。提示＝身「東洋服」／手「長手套」／腳「怪物獵人」／**項鍊「空賊」**（有飾品提示，base 70）。接受清單 **84 件（身 18／手 61／腳 4／項鍊 1）**，台服未開放 2 件（Hope Greaves [F]／[M]）。
  - **結果**：拿滿 MGP＝86 分・**101 金幣・無門檻・0 支染劑**（身「丹紅淑女浴衣」59＋手「魔女手套」42）；滿分＝100 分・**201 金幣**＋腳部一個門檻（火龍的鱗兌換，四件腳部接受裝備**全部**是怪物獵人聯動的火龍系列，沒有無門檻選項）。不想碰門檻的替代解要 **300,915 金幣**——因為得改用染色補分，而本週手/腳指定色是煤玉黑（要買兩支）。
  - **破口：「取得方式未收錄」被當成免費又無門檻**。腿部指定色羅蘭紫（7.21 新染劑，離線來源還沒收錄）在資料裡是 `gil=null, gate=null`，比較器只在第三順位的 `unknown` 件數才看得到它，於是第一版推薦選了它，標題寫「80 分・**42 金幣・無門檻**」——正是這套改版要消滅的自信假答案，與 week 443「116 金幣」、week 444「42 金幣」同一類。已在腳本加 `UNKNOWN_GATE`，**凡 `access === "none"` 一律掛門檻**，`GATE_GIL.unknown = 100,000`（排在所有說得出作法的門檻之後：使用者至少能照著「去打零式」行動，卻無法照著「不知道」行動）。修完同一週的最省解變成 101 金幣、真的無門檻。本週共 12 件命中（11 件裝備＋羅蘭紫染劑），現在都不會被靜靜當成免費，腳本每次也會把清單印出來當資料缺口的體溫計。
  - **順手修掉的顯示錯誤**：「沒有 NPC 固定價」區塊原本只有兩種說法，把**不可交易、取得方式未收錄**的羅蘭紫講成「市場板沒人上架」，把**貨幣兌換**的火龍護腿也講成同一句。改為四分支（有參考價／取得方式未收錄／可上市場板但無報價／沒有金幣標價）。裝備與染色列的 chip 也統一：`access === "none"` 只印一顆「⚠ 取得方式未收錄」，不再印一個空的「—」再疊一顆一模一樣的門檻標籤。
  - **render 回歸補漏**：第二套方案在分頁裡、要點了才畫，DOM stub 點不到，於是**門檻與「沒有 NPC 固定價」兩個區塊從來沒被測過**。harness 加一輪把兩個方案對調再渲染一次（順帶修掉 `import(mod + '#' + now)` 在兩個 case 共用同一個 `now` 時命中 module 快取、第二次根本不執行的問題）。七個狀態全過。
  - **驗收**：`validate-data` 0 error 0 warning、`sync-meta --apply`、`minify-data --apply`。
