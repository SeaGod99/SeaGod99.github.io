...
| **Teamcraft 料理／藥品** | `raw.githubusercontent.com/…/libs/data/src/lib/json/foods.json`（433 筆）與 `medicines.json`（135 筆） | `{ID, LevelItem, Bonuses:{Craftsmanship/Control/CP:{Relative, Value, ValueHQ, Max, MaxHQ}}}`，源自 `ItemFood` sheet | 建 [`data/craft-consumables.json`](../data/craft-consumables.json)。**只收含作業／加工／CP 加成的**（料理 49、藥品 12），採集用的 GP／獲得力不收；名稱與 patch 對 `items.json`，**查不到台服繁中名的不收**。加成是「百分比 ＋ 上限」兩段，且料理與藥品**各自從基礎值算再相加** |
